...
fun FirstStageScreen(navController: NavController, viewModel: DepositViewModel, modifier: Modifier = Modifier)
{
    ...
    )
    {
        ...
        Button(
            onClick = {
                if (viewModel.initialAmount.isBlank() || viewModel.periodMonths.isBlank() || viewModel.initialAmount.toInt() <= 0  ) {
                    ...
                    navController.navigate(ScreenRoutes.SecondStage.route)
                }
            },
        ...
        Button(
            onClick = {
                viewModel.clearData()
                navController.navigate(ScreenRoutes.MainActivity.route)
            },
            modifier = Modifier.padding(top = 16.dp)
        ) {
